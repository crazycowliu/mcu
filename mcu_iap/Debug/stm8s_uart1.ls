 ...
 634                     ; 129   BaudRate_Mantissa100 = (((uint32_t)CLK_GetClockFreq() * 100) / (BaudRate << 4));
 ...
 857                     ; 196 }
 860  027f 81            	ret
 986                     ; 211 void UART1_ITConfig(UART1_IT_TypeDef UART1_IT, FunctionalState NewState)
 986                     ; 212 {
 987                     	switch	.text
 ...
 995                     ; 213   uint8_t uartreg = 0, itpos = 0x00;
 999                     ; 216   assert_param(IS_UART1_CONFIG_IT_OK(UART1_IT));
...
1025  02b3               L46:
1026                     ; 217   assert_param(IS_FUNCTIONALSTATE_OK(NewState));
...
1045  02d0               L27:
1046                     ; 220   uartreg = (uint8_t)((uint16_t)UART1_IT >> 0x08);
...
2022                     ; 466   return (uint16_t)( (((uint16_t) UART1->DR) | temp ) & ((uint16_t)0x01FF));
...
2040  04e7 81            	ret
2074                     ; 474 void UART1_SendData8(uint8_t Data)
2074                     ; 475 {
2075                     	switch	.text
2076  04e8               _UART1_SendData8:
2080                     ; 477   UART1->DR = Data;
2082  04e8 c75231        	ld	21041,a
2083                     ; 478 }
2086  04eb 81            	ret
2120                     ; 486 void UART1_SendData9(uint16_t Data)
2120                     ; 487 {
2121                     	switch	.text
2122  04ec               _UART1_SendData9:
2124  04ec 89            	pushw	x
2125       00000000      OFST:	set	0
2128                     ; 489   UART1->CR1 &= ((uint8_t)~UART1_CR1_T8);
2130  04ed 721d5234      	bres	21044,#6
2131                     ; 491   UART1->CR1 |= (uint8_t)(((uint8_t)(Data >> 2)) & UART1_CR1_T8);
2133  04f1 54            	srlw	x
2134  04f2 54            	srlw	x
...
2254  0530 84            	pop	a
2255  0531 81            	ret
2289                     ; 528 void UART1_SetGuardTime(uint8_t UART1_GuardTime)
2289                     ; 529 {
2290                     	switch	.text
2291  0532               _UART1_SetGuardTime:
2295                     ; 531   UART1->GTR = UART1_GuardTime;
2297  0532 c75239        	ld	21049,a
2298                     ; 532 }
2301  0535 81            	ret
2335                     ; 556 void UART1_SetPrescaler(uint8_t UART1_Prescaler)
2335                     ; 557 {
2336                     	switch	.text
2337  0536               _UART1_SetPrescaler:
2341                     ; 559   UART1->PSCR = UART1_Prescaler;
2343  0536 c7523a        	ld	21050,a
2344                     ; 560 }
2347  0539 81            	ret
2491                     ; 568 FlagStatus UART1_GetFlagStatus(UART1_Flag_TypeDef UART1_FLAG)
2491                     ; 569 {
2492                     	switch	.text
...
2500                     ; 570   FlagStatus status = RESET;
2502                     ; 573   assert_param(IS_UART1_FLAG_OK(UART1_FLAG));
...
2536  0581               L242:
2537                     ; 577   if (UART1_FLAG == UART1_FLAG_LBDF)
2539  0581 1e02          	ldw	x,(OFST+1,sp)
2540  0583 a30210        	cpw	x,#528
2541  0586 2611          	jrne	L1601
2542                     ; 579     if ((UART1->CR4 & (uint8_t)UART1_FLAG) != (uint8_t)0x00)
...
2575  05ad               L3701:
2576                     ; 600       status = RESET;
2578  05ad 0f01          	clr	(OFST+0,sp)
2579  05af 200f          	jra	L7601
2580  05b1               L1701:
2581                     ; 605     if ((UART1->SR & (uint8_t)UART1_FLAG) != (uint8_t)0x00)
...
2592  05be               L1011:
2593                     ; 613       status = RESET;
2595  05be 0f01          	clr	(OFST+0,sp)
2596  05c0               L7601:
2597                     ; 617   return status;
2599  05c0 7b01          	ld	a,(OFST+0,sp)
2602  05c2 5b03          	addw	sp,#3
2603  05c4 81            	ret
2639                     ; 646 void UART1_ClearFlag(UART1_Flag_TypeDef UART1_FLAG)
2639                     ; 647 {
2640                     	switch	.text
2641  05c5               _UART1_ClearFlag:
2643  05c5 89            	pushw	x
2644       00000000      OFST:	set	0
2647                     ; 648   assert_param(IS_UART1_CLEAR_FLAG_OK(UART1_FLAG));
...
2665  05e3               L252:
2666                     ; 651   if (UART1_FLAG == UART1_FLAG_RXNE)
2668  05e3 1e01          	ldw	x,(OFST+1,sp)
2669  05e5 a30020        	cpw	x,#32
2670  05e8 2606          	jrne	L3211
2671                     ; 653     UART1->SR = (uint8_t)~(UART1_SR_RXNE);
2673  05ea 35df5230      	mov	21040,#223
2675  05ee 2004          	jra	L5211
2676  05f0               L3211:
2677                     ; 658     UART1->CR4 &= (uint8_t)~(UART1_CR4_LBDF);
2679  05f0 72195237      	bres	21047,#4
2680  05f4               L5211:
2681                     ; 660 }
2684  05f4 85            	popw	x
2685  05f5 81            	ret
2768                     ; 675 ITStatus UART1_GetITStatus(UART1_IT_TypeDef UART1_IT)
2768                     ; 676 {
2769                     	switch	.text
...
2777                     ; 677   ITStatus pendingbitstatus = RESET;
2779                     ; 678   uint8_t itpos = 0;
2781                     ; 679   uint8_t itmask1 = 0;
2783                     ; 680   uint8_t itmask2 = 0;
2785                     ; 681   uint8_t enablestatus = 0;
2787                     ; 684   assert_param(IS_UART1_GET_IT_OK(UART1_IT));
...
2815  062e               L262:
2816                     ; 687   itpos = (uint8_t)((uint8_t)1 << (uint8_t)((uint8_t)UART1_IT & (uint8_t)0x0F));
...
2831                     ; 689   itmask1 = (uint8_t)((uint8_t)UART1_IT >> (uint8_t)4);
2833  063f 7b04          	ld	a,(OFST+2,sp)
2834  0641 4e            	swap	a
2835  0642 a40f          	and	a,#15
2836  0644 6b02          	ld	(OFST+0,sp),a
2837                     ; 691   itmask2 = (uint8_t)((uint8_t)1 << itmask1);
...
3064                     	switch	.const
3065  0008               L371:
3066  0008 73746d38735f  	dc.b	"stm8s_stdperiph_dr"
3067  001a 697665725c73  	dc.b	"iver\src\stm8s_uar"
3068  002c 74312e6300    	dc.b	"t1.c",0
3069                     	xref.b	c_lreg
3070                     	xref.b	c_x
3090                     	xref	c_lursh
3091                     	xref	c_lsub
